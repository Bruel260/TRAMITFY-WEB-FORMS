...
// Control de logging para producción
function tramitfy_debug_log($message) {
    // Solo log de errores críticos en producción
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("Tramitfy: " . $message);
    }
}

// Validación de DNI/NIE español
function tramitfy_validate_dni($dni) {
    ...
    // NIE español (letra + 7 números + letra)
    if (preg_match('/^[XYZ][0-9]{7}[A-Z]$/', $dni)) {
        ...
        $conversions = ['X' => '0', 'Y' => '1', 'Z' => '2'];
        $numero_completo = $conversions[$primera] . $numero;
        
        $letras = "TRWAGMYFPDXBNJZSQVHLCKE";
        return ($letras[$numero_completo % 23] === $letra);
    }
        ...
        <div class="contrato-layout">
            ...
            <div class="contrato-form-panel">
                ...
                <div style="text-align: center; padding: 20px; border-top: 1px solid var(--border); margin-top: 20px;">
                    ...
                    <button
                        id="btn-test-email"
                        onclick="alert('Test de email deshabilitado para evitar duplicados')"
                        style="
                            background: #007bff;
                            color: white;
                            border: none;
    ...
    <script>
        ...
        const testData = {
            ...
            2: {
                ...
                vendedorEmail: 'vendedor@ejemplo.com'
            },
            3: {
                ...
                compradorEmail: 'comprador@ejemplo.com'
            },
            5: {
        ...
        function fillTestData() {
            ...
            fillField('vendedor-email', '');

            // PASO 3: Datos del comprador
            fillField('comprador-nombre', '');
        ...
        function testEmail() {
            alert('⚠️ Test de email deshabilitado para evitar emails duplicados');
            console.log('📧 Test de Email deshabilitado para evitar duplicados');
        }
...
function enviar_contrato_compraventa() {
    ...
    // Validar DNIs opcionales (pueden estar vacíos)
    if (!empty($_POST['vendedorDni']) && !tramitfy_validate_dni($_POST['vendedorDni'])) {
        wp_send_json_error('DNI del vendedor inválido');
        return;
    }
    
    if (!empty($_POST['compradorDni']) && !tramitfy_validate_dni($_POST['compradorDni'])) {
        wp_send_json_error('DNI del comprador inválido');
        return;
    }
...
function generar_y_enviar_contrato_pdf() {
    ...
    tramitfy_debug_log("Email procesado para contrato");

    if (!is_email($email)) {
        error_log("Tramitfy: ERROR - Email inválido");
        wp_send_json_error('Email inválido');
        return;
    }
    ...
    // Validar DNIs opcionales (pueden estar vacíos)
    if (!empty($_POST['vendedorDni']) && !tramitfy_validate_dni($_POST['vendedorDni'])) {
        error_log("Tramitfy: ERROR - DNI del vendedor inválido");
        wp_send_json_error('DNI del vendedor inválido');
        return;
    }
    
    if (!empty($_POST['compradorDni']) && !tramitfy_validate_dni($_POST['compradorDni'])) {
        error_log("Tramitfy: ERROR - DNI del comprador inválido");
        wp_send_json_error('DNI del comprador inválido');
        return;
    }
...
";
    ...
    // TEST BÁSICO DE EMAIL - ELIMINADO para evitar emails duplicados
    ...
    error_log("Tramitfy: Test email DESHABILITADO para evitar duplicados");
    ...
    try {
        ...
        // Buscar FPDF en múltiples ubicaciones
        $fpdf_paths = [
        ...
        foreach ($fpdf_paths as $fpdf_path) {
            if (file_exists($fpdf_path)) {
                ...
                $fpdf_loaded = true;
                error_log("Tramitfy: FPDF incluido desde: " . $fpdf_path);
                break;
            }
        }
        
        if (!$fpdf_loaded) {
            throw new Exception("FPDF library no encontrada en ninguna ubicación");
        }
...
Y en prueba de conformidad con cuanto antecede, firman el presente contrato por duplicado y a un solo efecto en el lugar y fecha indicados en el encabezamiento.';
    ...
    tramitfy_debug_log("Email enviado a destinatario");
    ...
    if ($sent) {
        ...
        tramitfy_debug_log("Enviando notificación interna");
...
function tramitfy_test_ajax() {
    // Verificar que solo admins puedan ejecutar tests
...
// Removido wp_ajax_nopriv para seguridad - solo admins pueden hacer tests

// Función de test de email - ELIMINADA para evitar emails duplicados
