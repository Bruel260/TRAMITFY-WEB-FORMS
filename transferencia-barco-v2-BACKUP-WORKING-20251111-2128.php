...
function tbv2_render_scripts() {
    ...
    async function createRedsysPayment(formData) {
        ...
        // 🔧 QUIRÚRGICO: NO enviar archivos en pago (causan error JSON)
        // Los archivos se enviarán después del pago exitoso
        console.log("🔧 QUIRÚRGICO: Creando pago SIN archivos para evitar errores");
        
        // Guardar archivos en memoria global para usar después del pago
        if (formData.files) {
            window.tbv2_saved_files = formData.files;
            console.log("💾 Archivos guardados en memoria:", Object.keys(formData.files));
        }
