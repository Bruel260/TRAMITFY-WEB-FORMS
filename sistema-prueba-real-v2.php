 ...
 */
function debug_log_aggressive($message, $context = 'DEBUG-V2') {
    $timestamp = date('Y-m-d H:i:s');
    $log_entry = "[$timestamp] [$context] $message\n";
    file_put_contents('/tmp/tramitfy-debug-v2.log', $log_entry, FILE_APPEND);
    file_put_contents('/tmp/tramitfy-all-debug.log', $log_entry, FILE_APPEND);
}

// Log cada acción importante
add_action('init', function() {
    if (isset($_POST['proceso_real'])) {
        debug_log_aggressive("=== FORMULARIO RECIBIDO ===");
        debug_log_aggressive("FILES recibidos: " . json_encode(array_keys($_FILES)));
        debug_log_aggressive("POST signature_data: " . (!empty($_POST['signature_data']) ? 'SÍ' : 'NO'));
    }
});
