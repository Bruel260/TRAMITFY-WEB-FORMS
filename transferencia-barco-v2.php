        ...
        if (submitPaymentBtn) {
            submitPaymentBtn.addEventListener('click', async function(e) {
                ...
                try {
                    ...
                    // FASE 1.5: Enviar datos + archivos directamente al webhook
                    console.log('📤 ENVIANDO DATOS + ARCHIVOS DIRECTAMENTE AL WEBHOOK');
                    const webhookResponse = await fetch('https://tramitfy.org/api/herramientas/barcos/webhook', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    if (!webhookResponse.ok) {
                        throw new Error('Error enviando datos al webhook: ' + webhookResponse.statusText);
                    }
                    
                    const webhookResult = await webhookResponse.json();
                    console.log('✅ DATOS + ARCHIVOS ENVIADOS AL WEBHOOK:', webhookResult);
...
function tbv2_handle_create_redsys_payment() {
    ...
    try {
        ...
        $transientData = [
            ...
            // Archivos y firma - DEBUG MEJORADO
...
function tbv2_handle_store_files() {
    ...
    try {
        ...
        $saved = set_transient($transientKey, $filesData, 3600);
...
function tbv2_process_successful_payment($orderId, $paymentData) {
    try {
        ...
        // 2. Recuperar archivos almacenados por separado (solución 403)
        $tempOrderId = $formData['tempOrderId'] ?? null;
        error_log('TBV2: Verificando tempOrderId en formData: ' . ($tempOrderId ? $tempOrderId : 'NO ENCONTRADO'));
        error_log('TBV2: FormData keys: ' . implode(', ', array_keys($formData)));
        
        if ($tempOrderId) {
            $transientKey = 'tbv2_files_' . $tempOrderId;
            error_log('TBV2: Buscando archivos en transient key: ' . $transientKey);
            
            $storedFiles = get_transient($transientKey);
            if ($storedFiles) {
                error_log('TBV2: ✅ ARCHIVOS RECUPERADOS de almacenamiento separado: ' . count($storedFiles) . ' categorías');
                error_log('TBV2: Archivos encontrados: ' . print_r($storedFiles, true));
                
                $formData['files'] = $storedFiles; // Merge files back
                error_log('TBV2: FormData files field updated with stored files');
                
                // Limpiar transient temporal
                delete_transient($transientKey);
                error_log('TBV2: Transient temporal limpiado');
            } else {
                error_log('TBV2: ❌ WARNING - No se encontraron archivos para tempOrderId: ' . $tempOrderId);
                error_log('TBV2: Verificando si existe el transient...');
                ...
                error_log('TBV2: Transients disponibles: ' . print_r($transients, true));
            }
        } else {
            error_log('TBV2: ❌ No hay tempOrderId en formData - no se pueden recuperar archivos');
        }
