...
function transferencia_barco_shortcode() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateForm() {
            ...
            if (formPages[currentPage].id === 'page-pago') {
                ...
                setTimeout(() => {
                    try {
                        // UNIFICADO: Usar paymentData para calcular monto Stripe
                        paymentData.updatePaymentMethod(itpMetodoPago);
                        paymentData.gestionamosITP = gestionamosITP;
                        
                        // Sincronizar ITP desde campos hidden si existe
                        const itpAmountField = document.getElementById('itp_amount');
                        const itpCommissionField = document.getElementById('itp_commission');
                        if (itpAmountField) {
                            const itpAmount = parseFloat(itpAmountField.value) || 0;
                            paymentData.updateITP(itpAmount);
                        }
                        
                        // Obtener monto unificado para Stripe
                        const stripeAmount = paymentData.stripeAmount;
                        
                        console.log('📌 [UNIFICADO] Cálculo de pago Stripe:');
                        console.log('  - Servicio base:', paymentData.servicePrice, '€');
                        console.log('  - Gestionamos ITP:', paymentData.gestionamosITP);
                        console.log('  - Método de pago ITP:', paymentData.itpMetodoPago);
                        console.log('  - ITP:', paymentData.itpAmount, '€');
                        console.log('  - Comisión ITP:', paymentData.itpCommission, '€');
                        console.log('  - TOTAL STRIPE:', stripeAmount, '€');
        ...
        function updateTransferTaxDisplay() {
            ...
            // UNIFICADO: Actualizar paymentData con ITP calculado
            paymentData.updateITP(itp);
        ...
        function updateTotal() {
            ...
            // UNIFICADO: Actualizar paymentData con valores calculados
            paymentData.tasas = baseTasas;
            paymentData.honorarios = discountedHonorarios;
            paymentData.iva = newIva;
            paymentData.updateITP(itp);
            paymentData.servicePrice = transferFee; // Precio base con servicios adicionales
            
            // Mantener compatibilidad con variables window para el resto del código
            window.currentTasas = baseTasas;
            window.currentHonorarios = discountedHonorarios;
            window.currentIva = newIva;

            // Cálculo final usando paymentData
            const totalGestion = baseTasas + discountedHonorarios + newIva + finalExtraFee;
            const total = paymentData.totalAmount; // Usar total unificado

            // 🔧 UNIFICADO: finalAmount ahora viene de paymentData
            ...
            // 🔧 UNIFICADO: Actualizar TODOS los displays usando sistema centralizado
            updateAllDisplays();
        }

        // ==================== SISTEMA UNIFICADO DE ACTUALIZACIÓN ====================
        function updateAllDisplays() {
            ...
            try {
                ...
                console.log('✅ [UNIFIED] Todos los displays actualizados correctamente', {
                ...
                console.error('❌ [UNIFIED] Error actualizando displays:', error);
            }
        }
        ...
        function collectFormData() {
            return {
                ...
                // Obtener precio base del modelo seleccionado
                basePrice: (() => {
                    const modelSelect = document.getElementById('model');
                ...
                // Servicios adicionales (usar variables globales existentes)
                cambioLista: window.cambioListaSeleccionado || false,
                cambioNombre: window.cambioNombreSeleccionado || false,
                cambioPuerto: window.cambioPuertoSeleccionado || false,
        ...
        function performCalculations(data) {
            ...
            // Cálculo valor fiscal (simplificado)
            ...
            if (data.cambioLista) calc.serviciosTotal += 64.95; // PRECIO_CAMBIO_LISTA
            if (data.cambioNombre) calc.serviciosTotal += 40.00; // PRECIO_CAMBIO_NOMBRE  
            if (data.cambioPuerto) calc.serviciosTotal += 40.00; // PRECIO_CAMBIO_PUERTO
            ...
            if (data.gestionamosITP && data.itpMetodoPago === 'transferencia') {
                // ITP por transferencia: Stripe solo cobra servicios
            ...
            console.log('📊 [CALC] Cálculos completados:', {
        ...
        function updateVehicleDisplays(data) {
            ...
            const vehicleElements = {
                ...
                'summary-vehicle-type': 'Embarcación',
        ...
        function updateServiciosAdicionalesDisplay(calc) {
            ...
            // Agregar servicios basándose en variables globales
            const servicios = [];
            if (window.cambioListaSeleccionado) {
                servicios.push({ nombre: 'Cambio de lista', precio: 64.95 });
            }
            if (window.cambioNombreSeleccionado) {
                servicios.push({ nombre: 'Cambio de nombre', precio: 40.00 });
            }
            if (window.cambioPuertoSeleccionado) {
                servicios.push({ nombre: 'Cambio de puerto base', precio: 40.00 });
            }
            
            servicios.forEach(servicio => {
                const serviceElement = document.createElement('div');
                serviceElement.style.cssText = 'display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #e5e7eb;';
                serviceElement.innerHTML = `
                    <div>
                        <div style="font-size: 15px; font-weight: 600; color: #1f2937;">${servicio.nombre}</div>
                        <div style="font-size: 13px; color: #6b7280; margin-top: 2px;">Servicio adicional</div>
                    </div>
                    <div style="font-size: 16px; font-weight: 700; color: #1f2937;">${servicio.precio.toFixed(2)} €</div>
                `;
                container.appendChild(serviceElement);
            });
        }

        function updateDesgloseUnified(calc, data) {
        ...
        function updateSummaryUnified(calc, data) {
            // Actualizar elementos del resumen de pago
            const summaryElements = {
                'summary-base-price': calc.servicioBase,
                'summary-tasas-gestion': calc.tasas,
                'summary-honorarios': calc.honorarios,
        ...
        function updateITPDisplays(calc, data) {
            // Actualizar todos los displays de ITP
            const itpElements = [
                'transfer_tax_display',
                'calculated-itp-display', 
                'sidebar-itp-amount'
        ...
        function syncAllGlobalVariables(calc) {
            ...
            console.log('✅ [SYNC] Variables globales sincronizadas', {
        ...
        function registerUnifiedEventListeners() {
            console.log('🔧 [UNIFIED] Registrando event listeners');
                ...
                if (element) {
                    events.forEach(eventType => {
                        element.addEventListener(eventType, () => {
                            console.log(`🔄 [TRIGGER] ${id} changed, updating all displays`);
                            updateAllDisplays();
                        });
                    });
                }
            ...
            // Servicios adicionales (botones)
            const servicioButtons = [
                'cambio-lista-si', 'cambio-lista-no',
                'cambio-nombre-si', 'cambio-nombre-no', 
                'cambio-puerto-si', 'cambio-puerto-no'
            ];
            ...
            servicioButtons.forEach(buttonId => {
                ...
                if (button) {
                    button.addEventListener('click', () => {
                        console.log(`🔄 [TRIGGER] ${buttonId} clicked, updating all displays`);
            ...
            console.log('✅ [UNIFIED] Event listeners registrados');
        }

        // ==================== INICIALIZACIÓN ==================== 
        // Registrar los event listeners cuando el DOM esté listo
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', registerUnifiedEventListeners);
        } else {
        ...
        function updatePaymentSummary() {
            ...
            // CORRECCIÓN: Separar tasas (19.05€) de honorarios correctamente
            // Las tasas son 19.05€, los honorarios dependen del servicio elegido
            const tasasReales = '19.05 €'; // Tasas DGT/gestoría
            const honorariosBase = gestionamosITP ? '155.94 €' : '115.94 €'; // 174.99-19.05 o 134.99-19.05
            const tasasHonorarios = tasasHonorariosEl ? tasasHonorariosEl.textContent : honorariosBase;
            const iva = ivaEl ? ivaEl.textContent : (gestionamosITP ? '32.74 €' : '24.34 €'); // 21% de honorarios
        ...
        if (manufacturerSelect) {
            ...
            manufacturerSelect.addEventListener('change', function() {
                ...
                basePrice = 0;
                onInputChange();
                // 🔧 UNIFICADO: Actualizar todos los displays cuando cambia fabricante
                if (typeof updateAllDisplays === 'function') {
                    updateAllDisplays();
                } else if (typeof updateTotal === 'function') {
                    updateTotal(); // Fallback
                }
                ...
                $csv_file = get_template_directory() . '/data.csv';
                if (!file_exists($csv_file)) {
                    // Si no está en el tema, buscar en el directorio del formulario
                    $csv_file = dirname(__FILE__) . '/data.csv';
                }

                $modelos_por_fabricante = array();

                if (file_exists($csv_file) && ($handle = fopen($csv_file, 'r')) !== false) {
                    // Saltar encabezado porque data.csv tiene header
                    fgetcsv($handle, 1000, ','); // Skip header: Fabricante,Modelo,Valor en Euros
                    while (($row = fgetcsv($handle, 1000, ',')) !== false) {
                        if (count($row) >= 3) {
                            $fab = trim($row[0]);
                            $mod = trim($row[1]);
        ...
        if (modelSelect) {
            ...
            modelSelect.addEventListener('change', function() {
                ...
                onInputChange();
                // 🔧 UNIFICADO: Actualizar todos los displays cuando cambia modelo
                if (typeof updateAllDisplays === 'function') {
                    updateAllDisplays();
                } else if (typeof updateTotal === 'function') {
                    updateTotal(); // Fallback
        ...
        if (submitPaymentBtn) {
            submitPaymentBtn.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    // UNIFICADO: Usar paymentData para calcular monto correcto
                    paymentData.gestionamosITP = gestionamosITP;
                    paymentData.updatePaymentMethod(itpMetodoPago);
                    paymentData.updateITP(currentTransferTax || 0);
                    
                    // Obtener monto unificado para Stripe
                    const paymentAmount = paymentData.stripeAmount;
                    
                    console.log('🔍 [UNIFICADO] VERIFICANDO CONDICIONES DE PAGO:');
                    console.log('  gestionamosITP:', paymentData.gestionamosITP);
                    console.log('  itpMetodoPago:', paymentData.itpMetodoPago);
                    console.log('  ITP:', paymentData.itpAmount, '€');
                    console.log('  Precio servicio:', paymentData.servicePrice, '€');
                    
                    if (gestionamosITP && itpMetodoPago === 'transferencia') {
                        console.log('💰 PAGO FRACCIONADO DETECTADO:');
                        console.log('   💳 Stripe cobra (servicio):', paymentAmount, '€');
                        console.log('   🏦 Cliente transfiere (ITP):', paymentData.itpAmount, '€');
                        console.log('   📊 Total trámite:', paymentData.totalAmount, '€');
                    } else {
    ...
    document.querySelectorAll('input[name="metodo_pago_itp"]').forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            if (itpMetodoPago === 'tarjeta') {
                itpComisionTarjeta = itpBaseAmount * 0.015;
    ...
    // ❌ FUNCIÓN ELIMINADA: actualizarPrecioFinal() 
    // Esta función causaba conflictos con el sistema unificado.
    // Ahora se usa updateAllDisplays() que maneja todo correctamente.
    ...
    function actualizarSidebarDinamico(pageId) {
        ...
        switch(pageId) {
            ...
            case 'page-pago':
                ...
                const serviciosExtras = document.querySelectorAll('.extra-option input[type="checkbox"]:checked');
                serviciosExtras.forEach(servicio => {
                    const label = servicio.closest('label');
                    if (label) {
                        const nombre = label.querySelector('.service-name')?.textContent || servicio.value;
                        const precio = parseFloat(servicio.dataset.price) || 0;
...
function tpb_send_emails_v2() {
    ...
    if ($itp_gestion === 'gestionan-ustedes' && $itp_metodo_pago === 'tarjeta') {
        ...
        error_log('[TPB] CORRECCIÓN ITP + tarjeta en EMAIL: final_amount recalculado = ' . $final_amount . ' (174.99 + ' . $itp_amount . ' + ' . $itp_comision . ')');
    }
...
function tpb_submit_form() {
    ...
    try {
        ...
        if ($itp_gestion === 'gestionan-ustedes' && $itp_metodo_pago === 'tarjeta') {
            $final_amount = 174.99 + $itp_amount + $itp_comision;
            tpb_debug_log('[TPB] CORRECCIÓN ITP + tarjeta: final_amount recalculado = ' . $final_amount . ' (174.99 + ' . $itp_amount . ' + ' . $itp_comision . ')');
        }
        ...
        if (!$no_encuentro && $vehicle_type !== 'Embarcación') {
            $csv_file = ($vehicle_type === 'Embarcación') ? 'data.csv' : 'data.csv';
        ...
        $form_data = array_merge(array(
            ...
            'serviciosAdicionalesTotal' => (string)(
                ($cambio_lista ? 64.95 : 0) + 
                (isset($_POST['cambioNombre']) ? 40.00 : 0) + 
                (isset($_POST['cambioPuerto']) ? 40.00 : 0)
            ),
