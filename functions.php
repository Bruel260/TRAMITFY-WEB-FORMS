...
if ( ! class_exists( 'Codevz_Core_Theme' ) ) {
	...
	class Codevz_Core_Theme {
		...
		public static function get_row_element( $i, $m = [] ) {
			...
			$data_settings = self::$preview ? " data-settings='" . wp_json_encode( $i, JSON_HEX_APOS ) . "'" : '';
			...
			if ( ! file_exists( str_replace( '-child', '', self::$dir ) . '-child/header.php' ) ) {
				...
				if ( $cover && $cover !== 'none' ) {
					...
					if ( $cover === 'rev' && $cover_rev ) {
						...
						} else if ( ! shortcode_exists( 'rev_slider' ) ) {
...
function cargar_fabricantes_modelos_js() {
    ?>
    <script>
    jQuery(document).ready(function($) {
        console.log('Script cargado correctamente');  // Verifica si el script se carga en la consola
    });
    </script>
    <?php
}
add_action('wp_footer', 'cargar_fabricantes_modelos_js');

function agregar_evento_conversion() {
    // Obtener la URL actual
    $current_url = home_url(add_query_arg(array(), $wp->request));

    // Verificar si es la página de éxito
    if ($current_url === "https://tramitfy.es/pago-realizado-con-exito/") {
        echo '<script>
            console.log("Evento de conversión ejecutado");
...
add_action('wp_footer', 'agregar_evento_conversion');



require_once get_template_directory() . '/puerto.php';
require_once get_template_directory() . '/hoja-asiento.php';
require_once get_template_directory() . '/renovacion-permiso.php';
require_once get_template_directory() . '/baja.php';
//require_once get_template_directory() . '/transferencia-propiedad.php';
require_once get_template_directory() . '/renovacion-titulaciones.php';
require_once get_template_directory() . '/matriculaciones.php';
require_once get_template_directory() . '/calculadora-itp-simple.php';
require_once get_template_directory() . '/nombre.php';
require_once get_template_directory() . '/leb.php';
