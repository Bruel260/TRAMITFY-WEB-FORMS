...
function tbv2_process_successful_payment($orderId, $paymentData) {
    try {
        ...
        // ⚡ FIX CRÍTICO: Si uploadedFiles está vacío, buscar archivos directamente en el directorio temporal
        if (empty($uploadedFiles)) {
            error_log("❌ TBV2: uploadedFiles vacío en transient, buscando archivos en directorio temporal");
            ...
            if (is_dir($temp_dir)) {
                $files = glob($temp_dir . '/*');
                error_log("🔍 TBV2: Archivos encontrados en $temp_dir: " . count($files));
                
                foreach ($files as $filePath) {
                    if (is_file($filePath)) {
                        $fileName = basename($filePath);
                        // Extraer inputId del nombre del archivo (formato: inputId_index_filename)
                        if (preg_match('/^([^_]+)_\d+_(.+)$/', $fileName, $matches)) {
                            $inputId = $matches[1];
                            ...
                            error_log("✅ TBV2: Archivo recuperado de directorio temporal - $originalName");
                        }
                    }
                }
            } else {
                error_log("❌ TBV2: Directorio temporal no existe: $temp_dir");
            }
        }
