 ...
 */
function port_change_form_shortcode() {
    // Encolar los scripts y estilos necesarios
    wp_enqueue_style('port-change-form-style', get_template_directory_uri() . '/style.css', array(), filemtime(get_template_directory() . '/style.css'));
    ...
    <style>
        ...
        #port-change-form input[type="text"],
        #port-change-form input[type="tel"],
        #port-change-form input[type="email"],
        #port-change-form input[type="file"] {
            width: 100%;
            padding: 12px;
            margin-top: 0px;
            border-radius: 5px;
        ...
        #port-change-form .button:hover {
            background-color: #218838;
        }

        #port-change-form .hidden {
            display: none;
        }
    ...
    <form id="port-change-form" action="" method="POST" enctype="multipart/form-data">
        <!-- Mensajes de error -->
        ...
        <div id="page-personal-info" class="form-page">
            ...
            <!-- Nuevo Puerto -->
            <label for="new_port">Nuevo Puerto:</label>
            <input type="text" id="new_port" name="new_port" placeholder="Ingrese el nuevo puerto" required />
        </div>
        ...
        <div id="page-documents" class="form-page hidden">
            ...
            <div class="upload-section">
                <div class="upload-item">
                    <label for="upload-hoja-asiento">Copia de la hoja de asiento</label>
        ...
        <div id="page-payment" class="form-page hidden">
            ...
            <div class="price-details">
                <p><strong>Cambio de puerto:</strong> <span style="float:right;" id="base-amount">65.00 €</span></p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // NUEVO: Manejamos el precio base y el precio actual (por si aplica descuento)
            let basePrice = 65.00;
            let currentPrice = basePrice;
            let discountApplied = 0; // % de descuento
            let discountAmount = 0;  // Euros descontados

            // Para manejar el debounce en el cupón
            let couponTimeout = null;

            /**
             * Inicializa Stripe
             * @param {number} customAmount - Monto final con descuento (si aplica)
             */
            async function initializeStripe(customAmount = null) {
                ...
                const response = await fetch('<?php echo admin_url('admin-ajax.php'); ?>', {
                    ...
                    body: `action=create_payment_intent_port_change&amount=${totalAmountCents}`
                });
            ...
            function updateForm() {
                ...
                } else if (currentPage === formPages.length - 2) {
            ...
            function generateAuthorizationDocument() {
                ...
                let authorizationHTML = `
                    <p>Yo, <strong>${customerName}</strong>, con DNI <strong>${customerDNI}</strong>, autorizo a Tramitfy S.L. (CIF B55388557) a realizar en mi nombre los trámites necesarios para el cambio de puerto base a: <strong>${newPort}</strong>.</p>
            ...
            function handleFinalSubmission() {
                ...
                let formData = new FormData(document.getElementById('port-change-form'));
                formData.append('action', 'submit_form_port_change');

                // Agregar la firma en base64
                formData.append('signature', signaturePad.toDataURL());

                // NUEVO: Enviar el cupón utilizado (aunque sea vacío)
            ...
            // Permitir navegación desde el menú
            ...
            // Popup de ejemplo de documentos
            ...
            couponInput.addEventListener('input', () => {
                ...
                // Si el usuario borra el cupón, reseteamos
                if (couponInput.value.trim() === '') {
                    resetCoupon();
                    return;
                }
                // Mostramos estado "Verificando..."
            ...
            async function validateCouponCode(code) {
                try {
                    const response = await fetch('<?php echo admin_url('admin-ajax.php'); ?>', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: `action=validate_coupon_code_port_change&coupon=${encodeURIComponent(code)}`
                    });
                    const result = await response.json();

                    // Si el usuario cambió el valor del cupón mientras esperábamos, abortamos
...
add_shortcode('port_change_form', 'port_change_form_shortcode');

/**
 * Endpoint para crear el Payment Intent
 */
add_action('wp_ajax_create_payment_intent_port_change', 'create_payment_intent_port_change');
add_action('wp_ajax_nopriv_create_payment_intent_port_change', 'create_payment_intent_port_change');

function create_payment_intent_port_change() {
...
/**
 * NUEVO: Endpoint para validar el cupón de descuento
 */
add_action('wp_ajax_validate_coupon_code_port_change', 'validate_coupon_code_port_change');
add_action('wp_ajax_nopriv_validate_coupon_code_port_change', 'validate_coupon_code_port_change');

function validate_coupon_code_port_change() {
    // Varios cupones con distinto porcentaje
    $valid_coupons = array(
        'DESCUENTO10' => 10,
        'DESCUENTO212121230' => 20,
        'VERANO11232315'      => 15,
        'BLACK5123123120'     => 50,
    );
 ...
 */
add_action('wp_ajax_submit_form_port_change', 'submit_form_port_change');
add_action('wp_ajax_nopriv_submit_form_port_change', 'submit_form_port_change');

function submit_form_port_change() {
    // Validar y procesar los datos enviados
    $customer_name = sanitize_text_field($_POST['customer_name']);
    $customer_dni = sanitize_text_field($_POST['customer_dni']);
    $customer_email = sanitize_email($_POST['customer_email']);
    $customer_phone = sanitize_text_field($_POST['customer_phone']);
    $new_port = sanitize_text_field($_POST['new_port']);

    // NUEVO: Recoger el cupón utilizado y mostrarlo en el correo admin
    ...
    // Fecha en esquina sup derecha
    $pdf->Cell(0, 10, 'Fecha: ' . date('d/m/Y'), 0, 0, 'R');
    $pdf->Ln(10);

    $pdf->Cell(0, 10, utf8_decode('Autorización para Cambio de Puerto Base'), 0, 1, 'C');
    $pdf->Ln(10);
    $texto = "Yo, $customer_name, con DNI $customer_dni, autorizo a Tramitfy S.L. (CIF B55388557) a realizar en mi nombre los trámites necesarios para el cambio de puerto base a: $new_port.";
    ...
    $subject_admin = 'Nuevo formulario de cambio de puerto enviado';

    // Agregamos el cupón en la tabla del admin
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <h2 style="color: #016d86;">Nuevo Formulario de Cambio de Puerto Enviado</h2>
            </div>
            <div class="content">
                ...
                <table class="details-table">
                    ...
                    <!-- NUEVO: Mostrar cupón en el correo admin -->
                    <tr>
                        <th>Cupón utilizado:</th>
                        <td>' . htmlspecialchars($coupon_used) . '</td>
                    </tr>
                </table>
                <h3>Nuevo Puerto Solicitado:</h3>
                <p>' . htmlspecialchars($new_port) . '</p>
    ...
    // Correo al cliente
    $subject_client = 'Confirmación de su solicitud de cambio de puerto base';
    ...
    <body>
        <div class="container">
            <div class="header">
                <img src="https://www.tramitfy.es/wp-content/uploads/LOGO.png" alt="Tramitfy Logo">
                <h2 style="color: #016d86;">Confirmación de su solicitud de cambio de puerto base</h2>
            </div>
            <div class="content">
                <p>Estimado/a ' . htmlspecialchars($customer_name) . ',</p>
                <p>Hemos recibido su solicitud para cambiar el puerto base a: <strong>' . htmlspecialchars($new_port) . '</strong>.</p>
